// Full header-only implementation of CIni class
#ifndef __INI_H__
#define __INI_H__
...
{
public:
    CIni() : m_pszPathName(NULL) {}
    CIni(LPCTSTR lpPathName) : m_pszPathName(NULL) { SetPathName(lpPathName); }
    virtual ~CIni() { if (m_pszPathName) delete[] m_pszPathName; }

    void SetPathName(LPCTSTR lpPathName) 
    {
        if (m_pszPathName) delete[] m_pszPathName;
        m_pszPathName = lpPathName ? _tcsdup(lpPathName) : NULL;
    }

    DWORD GetPathName(LPTSTR lpBuffer, DWORD dwBufSize) const 
    {
        if (!lpBuffer) return m_pszPathName ? (DWORD)_tcslen(m_pszPathName) : 0;
        _tcsncpy(lpBuffer, m_pszPathName, dwBufSize);
        return (DWORD)_tcslen(lpBuffer);
    }

#ifdef __AFXWIN_H__
    CString GetPathName() const { return CString(m_pszPathName); }
#endif

    DWORD GetString(LPCTSTR lpSection, LPCTSTR lpKey, LPTSTR lpBuffer, DWORD dwBufSize, LPCTSTR lpDefault = NULL) const 
    {
        if (lpBuffer) *lpBuffer = _T('\0');
    ...
    {
        if (!lpSection || !lpKey) return FALSE;
        return ::WritePrivateProfileString(lpSection, lpKey, lpValue ? lpValue : _T(""), m_pszPathName);
    }
    ...
    BOOL AppendString(LPCTSTR lpSection, LPCTSTR lpKey, LPCTSTR lpString) const 
    {
        ...
        LPTSTR pNewString = new TCHAR[_tcslen(psz) + _tcslen(lpString) + 1];
        _stprintf(pNewString, _T("%s%s"), psz, lpString);
        BOOL res = WriteString(lpSection, lpKey, pNewString);
        delete[] pNewString;
        delete[] psz;
        return res;
    }
    ...
    {
        int val = GetInt(lpSection, lpKey, 0, nBase);
        val += nIncrease;
        return WriteInt(lpSection, lpKey, val, nBase);
    }
    ...
    {
        if (!lpString || !lpFnStrProc) return FALSE;
        LPCTSTR p = lpString;
        DWORD dwLen = (DWORD)_tcslen(p);
        while (dwLen > 0) {
            if (!lpFnStrProc(p, lpParam)) return FALSE;
            p = &p[dwLen + 1];
    ...
    {
        if (!lpString || *lpString == _T('\0')) return bDefault;
        return (_tcsicmp(lpString, _T("true")) == 0 || _tcsicmp(lpString, _T("yes")) == 0 || _tcstol(lpString, NULL, BASE_DECIMAL) != 0);
    }
...
protected:
    ...
    {
        if (!lpSection || !lpKey)
            return lpDefault ? _tcsdup(lpDefault) : _tcsdup(_T(""));

        DWORD dwLen = DEF_PROFILE_THRESHOLD;
        LPTSTR psz = new TCHAR[dwLen + 1];
        DWORD dwCopied = ::GetPrivateProfileString(lpSection, lpKey, lpDefault ? lpDefault : _T(""), psz, dwLen, m_pszPathName);

        while (dwCopied + 1 >= dwLen) {
            dwLen += DEF_PROFILE_THRESHOLD;
            delete[] psz;
            psz = new TCHAR[dwLen + 1];
            dwCopied = ::GetPrivateProfileString(lpSection, lpKey, lpDefault ? lpDefault : _T(""), psz, dwLen, m_pszPathName);
        }

        return psz;
    }

    static DWORD __StringSplit(LPCTSTR lpString, LPTSTR lpBuffer, DWORD dwBufSize, LPCTSTR lpDelimiter, BOOL bTrimString = TRUE)
    {
